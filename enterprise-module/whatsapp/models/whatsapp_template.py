...
class WhatsAppTemplate(models.Model):
    ...
    def _get_template_button_component(self):
        ...
        if not self.button_ids and self.template_type != 'authentication':
            return None
        buttons = []
        if self.template_type == 'authentication':
            buttons.append({
    ...
    def button_submit_template(self):
        ...
        if self.template_type == 'authentication':
            json_vals['languages'] = [self.lang_code]
        else:
            json_vals['language'] = self.lang_code

        json_data = json.dumps(json_vals)
        templates_url = "message_templates" if self.template_type != 'authentication' else "upsert_message_templates"
        try:
            if self.wa_template_uid:
                wa_api._submit_template_update(json_data, self.wa_template_uid)
                self.status = 'pending'
            else:
                response = wa_api._submit_template_new(json_data, templates_url)
                self.write({
    ...
    def _get_body_component(self, free_text_json, template_variables_value):
        ...
        parameters = []
        free_text_count = 1
        for body_val in self.variable_ids.filtered(lambda line: line.line_type == 'body').sorted(key=lambda line: line._extract_variable_index()):
